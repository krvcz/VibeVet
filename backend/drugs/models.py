...
class BaseDrugModel(BaseAuditModel):
    ...
    measurement_value = models.DecimalField(max_digits=10, decimal_places=5, validators=[MinValueValidator(Decimal('0.00001'))])
    measurement_unit = models.ForeignKey(Unit, on_delete=models.PROTECT, related_name='%(class)s_measurement_unit')
    per_weight_value = models.DecimalField(max_digits=10, decimal_places=5, validators=[MinValueValidator(Decimal('0.00001'))])
    per_weight_unit = models.ForeignKey(Unit, on_delete=models.PROTECT, related_name='%(class)s_per_weight_unit')
